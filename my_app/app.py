    ...
    with st.sidebar:
        ...
        with st.expander("➕ 記錄演出行程"):
            with st.form("mini_add", clear_on_submit=True):
                ...
                m_map = st.text_input("場地 Google Map 網址 (選填)", placeholder="https://maps.google.com/...")
                ...
                <div class='row-countdown-box'>
                    <div style='display: flex; align-items: center; gap: 25px;'>
                        ...
                        <div>
                            ...
                            <span style='color:#8892b0; font-size:11px;'>{c['date'].strftime('%Y.%m.%d')} │ {c['location'].split(' ')[1] if ' ' in c['location'] else c['location']}</span>
                        </div>
                    </div>
        ...
        for c in upcoming_shows:
            ...
            <div class='schedule-row'>
                <div class='schedule-date-left'>
                    ...
                    <span style='font-size:11px; color:#444;'>{c['date'].year}</span>
                </div>
                <div class='schedule-detail-right'>
                    ...
                    <div style='display:flex; gap:8px; align-items:center; flex-wrap: wrap;'>
                        ...
                        {f"<a href='{final_map}' target='_blank' style='text-decoration:none;'><span class='location-tag' style='color:#9effeb; border-color:rgba(158,255,235,0.2);'>📍 Google Map</span></a>" if final_map and final_map != "#" else ""}
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
